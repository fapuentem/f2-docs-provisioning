...
1. *Install ~qmi_wwan~ Driver:*
   ...
   - *Manual Installation (if needed):* If the driver is not found, you
     ...
     #+BEGIN_SRC
       mkdir -p ~/tools
       git clone https://github.com/fapuentem/f2-maintenance-tools.git ~/tools/
     #+END_SRC
...
2. *Configure QMI Connection:* (using ~quectel-CM~):
   ...
   + Download the tar package
     ...
     #+BEGIN_SRC sh
       ...
       tar xvf ~/Downloads/quectel-CM.tar.gz
     #+END_SRC
   ...
   + Change ~metric~ variable in line 20 in ~default.script~.
     ...
     #+BEGIN_SRC sh
       sed -i '20s/^metric=0$/metric=2357/' ~/Downloads/quectel-CM/default.script
     #+END_SRC
   ...
   + Change ~UDHCPC_DIR~ should be ~/etc/udhcpc/~. And run ~install.sh~ as
     ...
     #+BEGIN_SRC sh
       sed -i 's|^UDHCPC_DIR=/usr/share/udhcpc$|UDHCPC_DIR=/etc/udhcpc|' ~/Downloads/quectel-CM/default.script
     #+END_SRC
